...
function AnalyticsPage() {
    ...
    (async () => {
      try {
        ...
        const [res, revRes] = await Promise.all([
          fetch("/api/call", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ function: "analytics.getAnalytics", args: {} }), credentials: "include" }),
          fetch("/api/call", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ function: "analytics.getRevenueTrend", args: {} }), credentials: "include" }),
        ]);
        const d = await res.json();
        const rt = await revRes.json();
        setData(d);
        setRevTrend(Array.isArray(rt) ? rt : []);
  ...
  // Key metrics
  const avgProjectSize = wonCount > 0 ? Math.round(Number(totalRevenue) / wonCount) : 0;
  const bestTrade = (byTrade||[]).length > 0 ? (byTrade as any[]).reduce((best: any, t: any) => t.win_rate > best.win_rate ? t : best, byTrade[0]) : null;
  const proposalToWinPct = totalEstimates > 0 ? Math.round((wonCount / totalEstimates) * 100) : 0;

  // Revenue trend
  const maxRevenue = Math.max(...revTrend.map((r: any) => Number(r.revenue) || 0), 1);
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-12">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
            ...
            <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">${Number(totalRevenue).toLocaleString()}</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Avg Project Size</p>
            <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">${avgProjectSize.toLocaleString()}</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Most Profitable Trade</p>
            <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100 capitalize">{bestTrade?.trade || "N/A"}</p>
            {bestTrade && <p className="text-xs text-gray-500">{bestTrade.win_rate}% win rate</p>}
          </div>
          <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Proposal-to-Win</p>
            <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{proposalToWinPct}%</p>
        ...
        {revTrend.length > 0 && (
          <div className="mt-10 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
            ...
            <p className="text-sm text-gray-500">Monthly won revenue</p>
            <div className="mt-4 flex items-end gap-1 h-40">
              {revTrend.map((r: any) => {
                const h = maxRevenue > 0 ? (Number(r.revenue || 0) / maxRevenue) * 100 : 0;
                const m = new Date(r.month).getMonth();
                return (
                  <div key={r.month} className="flex-1 flex flex-col items-center gap-1 group relative" style={{ minWidth: 0 }}>
                    <span className="text-[10px] text-gray-400 font-mono">${Math.round(Number(r.revenue || 0)).toLocaleString()}</span>
                    <div className="w-full bg-indigo-500 rounded-t group-hover:bg-indigo-600 transition-colors" style={{ height: `${Math.max(h, 2)}%` }}></div>
                    <span className="text-[10px] text-gray-400">{monthLabels[m]}</span>
                  </div>
                );
              })}
            </div>
            <p className="mt-2 text-right text-xs text-gray-500">Total: ${revTrend.reduce((s: number, r: any) => s + Number(r.revenue || 0), 0).toLocaleString()}</p>
          </div>
        )}
