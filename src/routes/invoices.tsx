  ...
  return (
    <div className="flex min-h-dvh flex-col">
      <header className="border-b border-gray-200 dark:border-gray-800">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4 text-sm font-medium">
            ...
            <Link to="/invoices" className="font-semibold text-indigo-600 dark:text-indigo-400">Invoices</Link>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-12">
        ...
        {invoices.length === 0 ? <div className="mt-10 rounded-xl border border-dashed border-gray-300 p-12 text-center dark:border-gray-700"><p className="text-gray-500">No invoices yet</p></div> : (
          <div className="mt-8 overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800"><table className="w-full text-sm"><thead className="bg-gray-50 dark:bg-gray-950"><tr><th className="px-4 py-3 text-left font-medium text-gray-500">Invoice #</th><th className="px-4 py-3 text-right font-medium text-gray-500">Total</th><th className="px-4 py-3 text-left font-medium text-gray-500">Status</th><th className="px-4 py-3 text-left font-medium text-gray-500">Date</th></tr></thead><tbody className="divide-y divide-gray-200 dark:divide-gray-800">{invoices.map((inv: any) => <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-950"><td className="px-4 py-3 font-medium">{inv.invoice_number}</td><td className="px-4 py-3 text-right">${Number(inv.total||0).toLocaleString()}</td><td className="px-4 py-3 capitalize">{inv.status||"draft"}</td><td className="px-4 py-3 text-gray-500">{inv.created_at ? new Date(inv.created_at).toLocaleDateString() : "—"}</td></tr>)}</tbody></table></div>
        )}
      </main>
    </div>
  );
